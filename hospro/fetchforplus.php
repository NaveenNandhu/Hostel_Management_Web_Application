...
if (isset($_GET['fetchbtn'])) {
    ...
    $sqlselect2 = "SELECT `$today` FROM `purchase_stock`";
    $resultselect2 = $connec->query($sqlselect2);
    $sqlselect3 = "SELECT `$today` FROM `daily_stock`";
    $resultselect3 = $connec->query($sqlselect3);
    if (!$resultselect2) {
        $create1 = "ALTER TABLE `purchase_stock` ADD COLUMN `$today` FLOAT(15);";
        $res_cr1 = $connec->query($create1);
    }
    if (!$resultselect3) {
        $create2 = "ALTER TABLE `daily_stock` ADD COLUMN `$today` FLOAT(15);";
    ...
    if ($resultselect) {
        ...
        foreach ($row as $rows) {
            ...
            <tr>
                ...
                <td>
                    <center>
                        <a href="#editmodal" class="btn btn-lg btn-success editbtn" data-toggle="modal"
                            style='height: 35px;width: 150px;font-size:16px;text-align:center;'><i class="fa-solid fa-circle-plus">
                                ADD</i></a>
                        ...
                        <div class="modal fade" id="deletetodmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <form method="post" action="deletetoday.php">
