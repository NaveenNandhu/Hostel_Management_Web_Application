...
if (isset($_GET['savechanges'])) {
    ...
    if ($quaa < 0) 
    {
        header("Location: dailystock.php?dateinput=$date2[0]&fetchbtn=");
    } 
    else 
    {
        if($quaa<=$check)
        {
        $sql2 = "SELECT `$newdate1` FROM `daily_stock` WHERE `S.No`=$id";
        ...
        $text2 = (float) $text2;
        $text2 += $quaa;
        $query_pur_tab = "UPDATE `daily_stock` SET `$newdate1`='$text2' WHERE `S.No`='$id'";
